...
def main(args: argparse.Namespace) -> None:
    ...
    # Load data
    logging.info(f"[1/6] Loading the data from {args.dataset}...")
    ...
    if re.search(r"IM-1393_BoneMarrow_TcellDep", adata.obs_names[0]):
        ...
        adata.obs['dictys_index'] = file_names_replaced
    else:
        adata.obs['dictys_index'] = adata.obs_names

    # Preprocess the adata
    logging.info(f"[1/4] Preprocess the adata...")
    for i, lin in enumerate(cell_selected.columns):
        ...
        expDF = pd.DataFrame(adata_lin.layers["counts"].A.T, index=adata_lin.var_names, columns=adata_lin.obs['dictys_index'])
        lin_dir = os.path.join(tmpSaveDir, lin, "data")
        os.makedirs(lin_dir, exist_ok=True)
        expDF.to_csv(os.path.join(lin_dir, "expression.tsv.gz"), sep="\t")
        
        # Link the data_fix_cell
        # mkdir data && find ../../data_fix_cell/Mono/ -maxdepth 1 -type f ! -name "expression.tsv.gz" -exec ln -s {} data/ \;
        data_fix_cell_dir = os.path.join(algoWorkDir, "data_fix_cell")
        run_command(f"find {os.path.join(data_fix_cell_dir, lin, 'data')} -maxdepth 1 ! -name 'expression.tsv.gz' -exec ln -s {{}} {lin_dir} \;")
        # Show the directory structure
        run_command(f"ls -R {lin_dir}")
        

    # Configure the makefile
    logging.info(f"[2/4] Configure the makefile...")
    for lin in cell_selected.columns:
        ...
        run_command(f"dictys_helper makefile_update.py {os.path.join(target_dir, 'config.mk')} '{{\"DEVICE\": \"cuda:0\", \"GENOME_MACS2\": \"hs\", \"JOINT\": \"1\"}}'")

    # Run Dictys
    logging.info(f"[3/4] Run Dictys...")
    for lin in cell_selected.columns:
